...
# Use the 'Sentence' column for SQL queries and the 'Label' column for labels
X = data['Sentence']
y = data['Label']
...
cnn_bilstm_predictions_strings = ['1' if pred == 0 else '0' for pred in cnn_bilstm_predictions]
 
# Convert numeric predictions to string labels
y_test_strings = ['1' if label == 0 else '0' for label in y_test]

# Calculate accuracy and classification report with string labels
cnn_bilstm_accuracy = accuracy_score(y_test_strings, cnn_bilstm_predictions_strings)
cnn_bilstm_report = classification_report(y_test_strings, cnn_bilstm_predictions_strings)
...
voting_predictions = voting_clf.predict(X_test_svd)
...
confusion_matrices = {
    ...
    'CNN-BiLSTM Hybrid': confusion_matrix(y_test, cnn_bilstm_predictions)
}
